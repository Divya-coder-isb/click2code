...
class ModelUnavailableError(ValueError):
    """Raised when Gemini responds with 503 UNAVAILABLE.

    The model is temporarily overloaded server-side. Subclasses
    `ValueError` for the same reason as `RateLimitError` — existing
    `ValueError` handlers keep working, while callers can branch on
    transient unavailability and display a softer warning.

    Model-unavailable errors are **never** auto-retried by this module —
    the caller is responsible for surfacing the error and letting the
    user retry manually (often after switching models).
    """
...
MODEL_UNAVAILABLE_MESSAGE = (
    "Gemini is temporarily unavailable due to high demand. This is a "
    "short-lived server-side issue. Please try again in a moment, or "
    "switch to a different model (e.g. gemini-2.0-flash) in the sidebar."
)


def _status_attrs(exc: BaseException):
    """Yield (attr_name, value) for the common HTTP-status attributes."""
    for attr in ("code", "status_code", "http_status", "status"):
        yield attr, getattr(exc, attr, None)
...
def _is_rate_limit_error(exc: BaseException) -> bool:
    ...
    """
    for _, value in _status_attrs(exc):
        if value == 429:
            return True
...
def _is_model_unavailable_error(exc: BaseException) -> bool:
    """Heuristic check for a Gemini 503 UNAVAILABLE error.

    Recognizes both the HTTP code 503 (via several attribute names) and
    the textual `UNAVAILABLE` status string that the Gemini error payload
    surfaces in `str(exc)`.
    """
    for _, value in _status_attrs(exc):
        if value == 503:
            return True
        if isinstance(value, str) and ("503" in value or value.upper() == "UNAVAILABLE"):
            return True

    message = str(exc)
    if "503" in message:
        return True
    # `UNAVAILABLE` is unique enough in a Gemini-error payload to be a
    # reliable signal. Avoid matching anywhere it'd appear naturally —
    # we only need to catch the `'status': 'UNAVAILABLE'` shape.
...
def _call_gemini(
    *,
    ...
    Raises:
        ...
        ModelUnavailableError: on Gemini 503 UNAVAILABLE errors.
            **Not** auto-retried — the user retries manually.
    ...
    try:
        ...
        # 503 UNAVAILABLE (transient server-side overload) — clean message,
        # never retry. The user retries manually after waiting / switching
        # models.
